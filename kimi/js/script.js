!function(_) {
    ...
    function updateDataNumber(data) {
        ...
        for(var i = 0; i < arr.length; i++) {
            ...
            if(arrSCG.length > 0) {
                ...
                var totalNumberForCat = 0;
                for(var j = 0;j < arrSCG.length; j++) {
                    arrSCG[j].unfinishedNumber = getUnfinishedNums(arrSCG[j].todos);
                    arrSCG[j].totalNumber = getTotalNums(arrSCG[j].todos);
                    unfinishedNumber += arrSCG[j].unfinishedNumber;
                    totalNumberForCat += arrSCG[j].totalNumber;
                }
                arr[i].unfinishedNumber = unfinishedNumber + getUnfinishedNums(arr[i].todos);
                arr[i].totalNumber = totalNumberForCat + getTotalNums(arr[i].todos);
                totalNumber += arr[i].unfinishedNumber;
                totalAllTasks += arr[i].totalNumber;
            } else {
        ...
        data.totalAllTasks = totalAllTasks;
    }
    // 获取任务总数节点
    ...
    // 获取第一个分类节点作为默认分类
    var firstCategory = cgList.querySelector('.category');
    console.log('First category element:', firstCategory);
    // 选中第一个分类节点
    if (firstCategory) {
        toSelectCG(firstCategory);
    } else {
    ...
    function toSelectCG(node,index) {
        console.log('toSelectCG called with node.index:', node.index, 'task index:', index);
        ...
        if(node.index.indexOf('-') == -1) {
            console.log('Selecting main category:', node.index, 'todos count:', data.category[node.index].todos.length);
            updateTaskList(data.category[node.index].todos,index);
        }else {
            var arr = node.index.split('-');
            console.log('Selecting subcategory:', arr[0], '-', arr[1], 'todos count:', data.category[arr[0]].subCategory[arr[1]].todos.length);
    ...
    $.delegate(cgList,'.delete','click',function() {
        ...
        if(index.indexOf('-') == -1) {
            ...
            // 检查主分类的直接任务
            for(var i = 0; i < category.todos.length; i++) {
                if(category.todos[i].isFinished === false) {
            ...
            if(!hasUnfinishedTasks) {
                for(var j = 0; j < category.subCategory.length; j++) {
                    for(var k = 0; k < category.subCategory[j].todos.length; k++) {
                        if(category.subCategory[j].todos[k].isFinished === false) {
                            hasUnfinishedTasks = true;
                            break;
                        }
                    }
                    if(hasUnfinishedTasks) break;
                }
            }
        } else {
            // 子分类
            ...
            for(var i = 0; i < subCategory.todos.length; i++) {
                if(subCategory.todos[i].isFinished === false) {
                    hasUnfinishedTasks = true;
                    break;
                }
            }
        ...
        if(hasUnfinishedTasks) {
            new Modal({text:'分类 "' + categoryName + '" 还有未完成的任务，不能删除！'})
    ...
    function toSelectTask(taskIndex) {
        ...
        console.log('toSelectTask - arrTodos length:', arrTodos.length, 'taskIndex:', taskIndex);
        console.log('Selected task:', arrTodos[taskIndex]);
        
        if (arrTodos[taskIndex]) {
    ...
    $.click(addTask,function() {
        console.log('addTask clicked - opening editor');
        gBody.appendChild(new Editor({name:'',date:format(new Date()),content:''}).on('confirm',function() {
            console.log('Editor confirm event triggered');
            var index = '';
            var todos = [];
            var obj = this.getData();
            console.log('Task data from editor:', obj);
            console.log('Selected category index:', selectCG.index);
            
            if(selectCG.index.indexOf('-') == -1) {
                todos = data.category[selectCG.index].todos;
                console.log('Adding to main category, todos before push:', todos.length);
            } else {
                var arr = selectCG.index.split('-');
                todos = data.category[arr[0]].subCategory[arr[1]].todos;
                console.log('Adding to subcategory, todos before push:', todos.length);
            }
            
            todos.push(obj);
            console.log('Task added, todos after push:', todos.length);
            
            todos.sort(byDate);
            index = todos.indexOf(obj);
            console.log('Task sorted and indexed at:', index);
            ...
            console.log('Task creation completed, data saved');
        }).container);
    });
    ...
    function renderTaskList(arr) {
        ...
        for(var i = 0;i < arr.length;i++) {
            ...
            if(oldTask !== null) {
                ...
                // 如果任务有数据，添加todo
                if(arr[i].name) {
                    var todoWidget = new Todo(arr[i]);
                    todoWidget.container.index = '' + arr[i].index;
                    task.taskWrap.appendChild(todoWidget.container);
                }
    ...
    function updateTaskList(arr,index) {
        ...
        var todos = taskList.querySelectorAll('.todo');
        if(todos.length > 0) {
            console.log('updateTaskList - todos found:', todos.length, 'requested index:', index);
            console.log('First todo index:', todos[0].index);
            if(index != undefined) {
                toSelectTask(index);
            } else {
            ...
            console.log('updateTaskList - no todos found');
    ...
    function renderCGList(arr) {
        ...
        for(var i = 0;i < arr.length;i++) {
            if(arr[i].name == '默认分类') {
                ...
                console.log('Created default category with id:', cg.category.id);
            } else {
                cg = new Category({name:arr[i].name,number:arr[i].number});
            }
            // 在category节点上添加index
            cg.category.index = '' + i;
            console.log('Created category:', arr[i].name, 'with index:', cg.category.index);
            var arrSCG = arr[i].subCategory;
            if(arrSCG.length > 0){
                for(var j = 0;j < arrSCG.length; j++) {
    ...
    function updateCGList(arr,index,taskIndex) {
        console.log('updateCGList called with index:', index, 'taskIndex:', taskIndex);
        cgList.replaceChild(renderCGList(arr),cgList.children[1]);
        if(typeof index == 'undefined') {
            console.log('No index provided, selecting first category');
            var firstCategory = cgList.querySelector('.category');
            if (firstCategory) {
                toSelectCG(firstCategory);
            }
            ...
            console.log('Found', lis.length, 'category list items');
            if(index.indexOf('-') == -1) {
                console.log('Selecting main category at index:', index);
                toSelectCG(lis[index].querySelector('.category'),taskIndex);
            }else {
                var array = index.split('-');
                console.log('Selecting subcategory:', array[0], '-', array[1]);
    ...
    function updateTotalTaskNums() {
        // 显示格式改为 (未完成数/总数)
        var unfinished = data.totalNumber || 0;
        var total = data.totalAllTasks || 0;
        totalTaskNums.innerText = unfinished + '/' + total;
    }
}(util);