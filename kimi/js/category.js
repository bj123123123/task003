!function(_) {
    ...
    _.extend(Category.prototype,{
        ...
        setContent: function(obj){
            ...
            // 显示格式改为 (未完成数/总数)
            var unfinished = obj.unfinishedNumber || obj.number || 0;
            var total = obj.totalNumber || obj.number || 0;
            this.cgNumber.innerText = unfinished + '/' + total;
        },
        // 添加子类
        ...
        updateNumber: function() {
            ...
            if(subs.length > 0) {
                for(var i = 0; i < subs.length; i++) {
                    var text = subs[i].innerText;
                    var parts = text.split('/');
                    if(parts.length === 2) {
                        unfinishedNum += Number(parts[0]);
                        totalNum += Number(parts[1]);
                    } else {
                        unfinishedNum += Number(text);
                        totalNum += Number(text);
                    }
                }
                this.cgNumber.innerText = unfinishedNum + '/' + totalNum;
            }
        },
        // 删除分类
    ...
    _.extend(SubCategory.prototype,{
        ...
        setContent: function(obj){
            ...
            var unfinished = obj.unfinishedNumber || obj.number || 0;
            var total = obj.totalNumber || obj.number || 0;
            this.scgNumber.innerText = unfinished + '/' + total;
        },
        // 添加兄弟子类
        ...
        // 更新子类数量
        updateNumber: function(unfinished, total) {
            this.scgNumber.innerText = unfinished + '/' + total;
        },
        // 删除子类
