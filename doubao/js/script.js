!function(_) {
    ...
    function updateDataNumber(data) {
        ...
        for(var i = 0; i < arr.length; i++) {
            ...
            var categoryUnfinished = getUnfinishedNums(arr[i].todos);
            var categoryTotal = getTotalNums(arr[i].todos);
            if(arrSCG.length > 0) {
                var number = 0;
                var total = 0;
                for(var j = 0;j < arrSCG.length; j++) {
                    ...
                    number += arrSCG[j].unfinishedNumber;
                    total += arrSCG[j].totalNumber;
                }
                arr[i].unfinishedNumber = number + categoryUnfinished;
                arr[i].totalNumber = total + categoryTotal;
                totalNumber += arr[i].unfinishedNumber;
                totalTasks += arr[i].totalNumber;
            } else {
                arr[i].unfinishedNumber = categoryUnfinished;
                arr[i].totalNumber = categoryTotal;
                totalNumber += arr[i].unfinishedNumber;
                totalTasks += arr[i].totalNumber;
            }
        }
    ...
    $.delegate(cgList,'.delete','click',function() {
        ...
        if(index.indexOf('-') == -1) {
            ...
            for(var i = 0; i < category.todos.length; i++) {
                if(!category.todos[i].isFinished) {
                    hasUnfinished = true;
                    break;
                }
            }
            ...
            if(!hasUnfinished) {
                for(var j = 0; j < category.subCategory.length; j++) {
                    ...
                    for(var k = 0; k < subCategory.todos.length; k++) {
                        if(!subCategory.todos[k].isFinished) {
                            hasUnfinished = true;
                            break;
                        }
                    }
            ...
            for(var i = 0; i < subCategory.todos.length; i++) {
                if(!subCategory.todos[i].isFinished) {
                    hasUnfinished = true;
                    break;
                }
            }
        ...
        if(hasUnfinished) {
            var modal = new Modal({text:'该分类下有未完成任务，无法删除！'});
            modal.appendTo(document.body);
            // 移除取消按钮
            var cancelBtn = modal.container.querySelector('.cancel');
            ...
            .on('confirm',function() {
                if(this.input.value.trim() != ''){
                    ...
                    if(selectCG.className.indexOf('sub-category') != -1) {
                        ...
                        index = '' + (data.category.push({name:this.input.value,unfinishedNumber:0,totalNumber:0,todos:[],subCategory:[]}) - 1);
                    }else {
                        // 否则，在数据中当前分类下添加子类
                        // data.category[selectCG.index].subCategory.push({name:this.input.value,number:0,todos:[]});
                        index = selectCG.index + '-' + (data.category[selectCG.index].subCategory.push({name:this.input.value,unfinishedNumber:0,totalNumber:0,todos:[]}) - 1);
                    }
                    // 更新分类列表
    ...
    function renderCGList(arr) {
        ...
        for(var i = 0;i < arr.length;i++) {
            ...
            cg.category.index = '' + i;
            // 修改任务数显示格式为(未完成数/总数)
            cg.cgNumber.innerText = arr[i].unfinishedNumber + '/' + arr[i].totalNumber;
            var arrSCG = arr[i].subCategory;
            if(arrSCG.length > 0){
                for(var j = 0;j < arrSCG.length; j++) {
                    ...
                    // 修改子分类任务数显示格式为(未完成数/总数)
                    subCG.scgNumber.innerText = arrSCG[j].unfinishedNumber + '/' + arrSCG[j].totalNumber;
    ...
    function updateTotalTaskNums() {
        totalTaskNums.innerText = data.totalNumber + '/' + data.totalTasks;
    }
}(util);